...
class ChatBot:
    def __init__(self, token, message, url):
        self.token = token
    ...
    def run(self):
        while True:
            try:
                ...
                bot.send_message(self.message.chat.id, "Захожу най сайт hh...")
                ...
                bot.send_message(self.message.chat.id, "Ввожу данные...")
                print("Ввожу данные...")
                time.sleep(2)
                ...
                button_enter.click()
                bot.send_message(self.message.chat.id, "Я залогинился!")
                print("Я залогинился!")

                # click on "Моё резюме"
                bot.send_message(self.message.chat.id, 'Захожу в раздел "Моё резюме"...')
                ...
                for button in all_buttons:
                    if button.text == "Поднять в поиске":
                        button.click()
                        bot.send_message(self.message.chat.id, "Я поднял резюме!")
                        print("Я поднял резюме!")
                        all_buttons_list.append(button.text)

                if "Поднять в поиске" not in all_buttons_list:
                    bot.send_message(self.message.chat.id, "Кнопки не найдены попробуйте позже.")
                ...
                bot.send_message(self.message.chat.id, "Я вышел!")
                info_list["session"] += 1
                bot.send_message(self.message.chat.id, f'Сессия {info_list["session"]} завершена! Повтор через 2 ч.')
...
if __name__ == '__main__':
    ...
    def stet_3(message):
        ...
        chat_bot = ChatBot(token=conf.TOKEN, message=message, url=url_page)
        chat_bot.run()
